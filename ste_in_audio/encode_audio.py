...
def encode_audio():
    ...
    elif (messages.get('1.0','end').rstrip('\n') == ''):
        messagebox.showerror("Errors!","Vui lòng nhập thông điệp cần giấu!")
    elif (password.get() == ''):
    ...
    elif (name_audio_save.get('1.0','end').rstrip('\n') == ''):
        messagebox.showerror("Errors!","Vui nhập tên file!")
    else:
        check = hiding_audio.encoded_hiding_audio(dir_audio.get('1.0','end').rstrip('\n'),messages.get('1.0','end').rstrip('\n'),dir_folder.get('1.0','end').rstrip('\n')+name_audio_save.get('1.0','end').rstrip('\n'))
        if (check == True):
            messagebox.showinfo("Succesfull!","Giấu tin vào Audio thành công!")
        else:
            messagebox.showerror("Errors!","Giấu tin vào Audio thất bại!")
menubar = Menu(root)
...
# Enter the Messages
l3 = Label(root, text="Nhập Thông Điệp ", bg="#f2f2f2",fg="black",font="(Arial)")
...
l10 = Label(root, text="Nhập Tên Audio ", bg="#f2f2f2",fg="black",font="(Arial)")
l10.grid(row=18,column=0,pady=10)
name_audio_save=Text(root,width=30,height=1)
name_audio_save.grid(row=18,column=1)

#Encoded
btn_open = Button(root,width=10, text="Encoded", fg="red",activebackground = "red", command=encode_audio)  
